    ...
    <>
      ...
      <section className="py-20 md:py-28">
        <div className="mx-auto grid max-w-[1200px] gap-10 px-6 md:px-8 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20 lg:px-12">
          <div>
            <p className="mb-4 text-[0.75rem] font-medium capitalize italic tracking-[0.08em] text-rose-text">
              The Rella standard
            </p>
            <h2 className="text-3xl font-bold uppercase leading-tight tracking-[0.06em] text-rose-text md:text-5xl">
              The best plan starts with a real conversation.
            </h2>
          </div>
          <div>
            <p className="mb-5 text-lg font-light leading-relaxed text-ink/70">
              Rella is built around a simple idea: people should feel heard before they are asked
              to make a decision. That means beginning with the goal, explaining the options
              plainly, and recommending only the next step that makes sense for the individual.
            </p>
            ...
            <div className="mt-8 flex flex-wrap gap-2">
              {careAreas.map((area) => (
                <span
                  key={area}
                  className="border-[1.5px] border-rose bg-white px-4 py-3 text-xs font-bold uppercase tracking-[0.08em] text-ink"
                >
                  {area}
                </span>
              ))}
      ...
      <section className="py-20 md:py-28">
        <div className="mx-auto grid max-w-[1200px] gap-6 px-6 md:grid-cols-2 md:px-8 lg:px-12">
          <Link href="/locations/vacaville" className="group border-y border-rose/35 p-7 transition-colors hover:border-rose md:p-9">
            <p className="mb-3 text-[0.75rem] font-medium capitalize italic tracking-[0.08em] text-ink">Vacaville</p>
            <h2 className="mb-3 text-2xl font-bold uppercase tracking-[0.06em] text-rose-text">542 Main Street</h2>
            <p className="text-sm text-ink/70">Explore the clinic, hours, services, and directions <span aria-hidden="true">→</span></p>
          </Link>
          <Link href="/locations/napa" className="group border-y border-rose/35 p-7 transition-colors hover:border-rose md:p-9">
            <p className="mb-3 text-[0.75rem] font-medium capitalize italic tracking-[0.08em] text-ink">Napa</p>
            <h2 className="mb-3 text-2xl font-bold uppercase tracking-[0.06em] text-rose-text">1541 3rd Street</h2>
            <p className="text-sm text-ink/70">Explore the clinic, hours, services, and directions <span aria-hidden="true">→</span></p>
          </Link>
        </div>
      </section>
