...
import VacavilleBotoxPage, { metadata } from "./page";
import { resolveBookingHref } from "@/lib/booking-routes";

const html = renderToStaticMarkup(<VacavilleBotoxPage />);
const text = html
  .replace(/<!-- -->/g, "")
  .replace(/<[^>]+>/g, " ")
  .replace(/\s+/g, " ")
...
describe("Vacaville Botox local-acquisition page", () => {
  it("uses unique, exact index metadata", () => {
    expect(metadata.title).toBe("Botox in Vacaville, CA | Pricing & Visit Guide");
    expect(metadata.alternates?.canonical).toBe(
      "https://experiencerella.com/vacaville/botox",
    );
  ...
  it("keeps every booking action on the custom Vacaville Tox entry", () => {
    ...
    expect(destination.searchParams.get("service")).toBe("botox");
    expect(expected).not.toContain("dashboard.boulevard.io");
  });

  it("shows approved pricing, location, hours, and no Napa-only deposit promise", () => {
    for (const fact of [
      "$18/unit",
      "$6/unit",
      "$13/unit",
    ...
    expect(text).not.toContain("$50 deposit");
    expect(text).not.toContain("complimentary touch-up");
  ...
  it("has one H1 and no patient-data form", () => {
