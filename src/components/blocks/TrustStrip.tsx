...
export function TrustStrip({ items, ariaLabel, className = "" }: TrustStripProps) {
  return (
    <section aria-label={ariaLabel} className={`border-y border-ink/15 bg-rose ${className}`}>
      <div
        className="mx-auto grid max-w-[1160px] grid-cols-2 gap-px bg-ink/15 md:grid-cols-4"
      >
        {items.map((item) => (
          <p
            key={item}
